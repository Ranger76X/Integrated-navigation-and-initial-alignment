...
D_GYR = [B_SensorData(:,1),B_SensorData(:,2),B_SensorData(:,3)]';          %按照算法顺序重排陀螺仪三轴读数顺序pitch,row,yaw                                                                         %                             x    y    z
...
Delta_sita = [0;0;0];                                                      %角增量
Euler(:,1) = [45/180*pi;0;0];                                              %初始欧拉角pitch,yaw,row(弧度制)
qbn(:,1) = Euler2Quaternion(Euler(1),Euler(2),Euler(3));                   %初始姿态四元数,东北天坐标系，旋转顺序(zxy)312
Cbn(:,:,1) = Quaternion2ACM(qbn(:,1));                                     %初始姿态旋转矩阵,东北天坐标系，旋转顺序(zxy)312
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%位姿初始化%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i = 2:D_N                                          
%--------------------------------1.姿态矩阵解算-----------------------------
