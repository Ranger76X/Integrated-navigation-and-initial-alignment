...
rw = r_walk(1:D_N,1);
rw = zeros(D_N,1);
noise_gyr = [0;mvnrnd(0,0.000005,D_N-1)];
...
% Eulerbb(:,1) = [45/180*pi;0;0];                                            %åˆå§‹æ¬§æ‹‰è§’pitch,yaw,row(å¼§åº¦åˆ?)
% qbb(:,1) = Euler2Quaternion(Eulerbb(1),Eulerbb(2),Eulerbb(3));             %åˆå§‹å§¿æ?å››å…ƒæ•°,ä¸œåŒ—å¤©åæ ‡ç³»ï¼Œæ—‹è½¬é¡ºåº?(zxy)312
...
%         
%     end
%     storeSVD(j,:) = Euler(:,end);
%     storeSVD2(j,:) = Euler(:,end)-[45;75;-30];
...
%         display([num2str(i),':xbest=[',num2str(BAS.xbest'),'],fbest=',num2str(BAS.fbest)])
    ...
    N = 60;%éœ?è¦çš„è§‚æµ‹çŸ¢é‡å¯¹æ•°
    BAS2.eta = 0.9;
    BAS2.c = 1;                  %ratio between step and d0
    BAS2.step = 1;               %initial step set as the largest input range
    BAS2.n = 500;                %iterations
    BAS2.k = 3;                  %space dimension
    BAS2.x = [pitch;roll;yaw];        %intial value
    BAS2.xbest = BAS2.x;
    BAS2.f = f3_2(BAS2.x,N,vb,Beta);
    BAS2.fbest = BAS2.f;
    BAS2.fbest_store = BAS2.fbest;
    BAS2.x_store = [0;BAS2.x;BAS2.fbest];

    for i = 1:BAS2.n
        BAS2.d0=BAS2.step/BAS2.c;
        BAS2.dir=rands(BAS2.k,1);
        BAS2.dir=BAS2.dir/(eps+norm(BAS2.dir));    

        BAS2.xleft=BAS2.x+BAS2.dir*BAS2.d0;
        BAS2.fleft=f3_2(BAS2.xleft,N,vb,Beta);
        BAS2.xright=BAS2.x-BAS2.dir*BAS2.d0;
        BAS2.fright=f3_2(BAS2.xright,N,vb,Beta);    

        BAS2.x=BAS2.x-BAS2.step*BAS2.dir*sign(BAS2.fleft-BAS2.fright);

        BAS2.f = f3_2(BAS2.x,N,vb,Beta);

        %%%%%%%%%%
        if BAS2.f<BAS2.fbest
            BAS2.xbest=BAS2.x;
            BAS2.fbest=BAS2.f;        
        end
        %%%%%%%%%%%
        BAS2.x_store=cat(2,BAS2.x_store,[i;BAS2.x;BAS2.f]);
        BAS2.fbest_store=[BAS2.fbest_store;BAS2.fbest];
        BAS2.xbest_store(i,:) = BAS2.xbest; 
%         display([num2str(i),':xbest=[',num2str(BAS.xbest'),'],fbest=',num2str(BAS.fbest)])
        %%%%%%%%%%%
        if(BAS2.fbest >= 0.4)
            BAS2.step=BAS2.step;
        else
            BAS2.step=BAS2.step*BAS2.eta;
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %æ•°æ®æ˜¾ç¤ºéƒ¨åˆ†
...
% err_pitch3 = sum(abs(storeSVD2(:,1)))/100;
% err_roll3 = sum(abs(storeSVD2(:,2)))/100;
% err_yaw3 = sum(abs(storeSVD2(:,3)))/100;
