...
C_yaw  =  [
           ...
           -sin(yaw) cos(yaw)   0;
                0        0      1;
          ];%在弹道坐标系下改变射向,绕弹道Y轴旋转,对应导航坐标系下的pitch矩阵
...
figure(3);
clf(3);
plot3(GPS_Y(:,1),GPS_Y(:,2),GPS_Y(:,3),'K.');  
%%%%%%%%%%%%%%%%%%%%%%%%%%地球模型(WGS84简化参数)%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
WGS84.a = 6371100;                                                         %长半轴
WGS84.b = 6371100;                                                         %短半轴
...
Cbn(:,:,1) = eye(3);
qbn(:,1) = [1;0;0;0];
Euler(:,1) = [0;0;0];%初始欧拉角pitch,yaw,row(弧度制)
...
n = 100;
for i = 2:n
    jaco(:,i) = Cal_Jaco(N,Euler(1,i-1),Euler(2,i-1),Euler(3,i-1),vb,Beta);
    Euler(:,i) = Euler(:,i-1) + step*[jaco(1,i);jaco(2,i);jaco(3,i)];
end
