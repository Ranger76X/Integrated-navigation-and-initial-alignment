...
noise = [mvnrnd(0,0.000005,D_N)];

noise_gyr1 = [mvnrnd(0,0.0000005,D_N)]+rw;
noise_gyr2 = [mvnrnd(0,0.0000005,D_N)]+rw;
noise_gyr3 = [mvnrnd(0,0.0000005,D_N)]+rw;
noise_gps1 = mvnrnd(0,0.02,D_N);
noise_gps2 = mvnrnd(0,0.02,D_N);
noise_gps3 = mvnrnd(0,0.02,D_N);
noise_acc1 = mvnrnd(0,0.000001,D_N)+rw;
noise_acc2 = mvnrnd(0,0.000001,D_N)+rw;
noise_acc3 = mvnrnd(0,0.000001,D_N)+rw;

% noise_gyr = zeros(D_N,1);
% noise_gps = zeros(D_N,1);
% noise_acc = zeros(D_N,1);

D_ACC = [B_SensorData(:,4)+noise_acc1,B_SensorData(:,5)+noise_acc2,B_SensorData(:,6)+noise_acc3]';
D_GYR = [B_SensorData(:,1)+noise_gyr1,B_SensorData(:,2)+noise_gyr2,B_SensorData(:,3)+noise_gyr3]';
GPS_Y = ([Y(:,1)+noise_gps1,Y(:,2)+noise_gps2,Y(:,3)+noise_gps3]')';
GPS_V = ([Y(:,4)+noise_gps1/10,Y(:,5)+noise_gps2/10,Y(:,6)+noise_gps3/10]')';
...
BAS.xbest = BAS.x;
BAS.f = f6(BAS.x,N,vb,Beta,pitch,yaw);
BAS.fbest = BAS.f;
BAS.fbest_store = BAS.fbest;
BAS.x_store = [0;BAS.x;BAS.fbest];
...
for i = 1:BAS.n
    ...
    BAS.xleft=BAS.x+BAS.dir*BAS.d0;
    BAS.fleft=f6(BAS.xleft,N,vb,Beta,pitch,yaw);
    BAS.xright=BAS.x-BAS.dir*BAS.d0;
    BAS.fright=f6(BAS.xright,N,vb,Beta,pitch,yaw);   
