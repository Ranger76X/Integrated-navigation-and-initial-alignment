...
for i = 5:D_N                                          
%--------------------------------1.姿态矩阵解算-----------------------------
%     Delta_sita = (D_GYR(:,i)+3*D_GYR(:,i-1)+3*D_GYR(:,i-2)+D_GYR(:,i-3)) * D_T/8;           %上一历元角速度增量                            
%     Delta_sita_1 = (D_GYR(:,i-3)+D_GYR(:,i-2)) * D_T/6;                    %上上一历元角速度增量
%     Delta_sita_2 = (D_GYR(:,i-2)+D_GYR(:,i-1)) * D_T/6;                      %上上一历元角速度增量
%     Delta_sita_3 = (D_GYR(:,i-1)+D_GYR(:,i)) * D_T/6;                    %上上一历元角速度增量
    
    
    qbn(:,i-3) = AttitudeUpdate3(qbn(:,i-4),Delta_sita,Delta_sita_1,Delta_sita_2,Delta_sita_3);
    
    Euler(:,i-3) = Quaternion2Euler(qbn(:,i-3));
%     Cbn(:,:,i) = 
%--------------------------------2.速度更新--------------------------------
    Delta_v = D_GYR(:,i) * D_T;
