...
%     K1.K_Perr(:,i) = [GPS_Y(i-1,1) - XYZ(1,i);GPS_Y(i-1,2) - XYZ(2,i);GPS_Y(i-1,3) - XYZ(3,i)];
    ...
    ek = (K1.K_R^(-1/2))*[K1.K_Perr(:,i); K1.K_Verr(:,i)];
    if norm(ek) > 1
        weight_ek = 1/norm(ek);
    else 
        weight_ek = 1;
    end
    K1.K_R = (K1.K_R^(1/2))*(1/weight_ek*eye(6))*(K1.K_R^(1/2));
...
Z_Err(:,1) = Y(9:10:end-1,3)'-XYZ(1,10:10:end);   %x
Z_Err(:,2) = Y(9:10:end-1,1)'-XYZ(2,10:10:end);   %y
Z_Err(:,3) = Y(9:10:end-1,2)'-XYZ(3,10:10:end);   %z
