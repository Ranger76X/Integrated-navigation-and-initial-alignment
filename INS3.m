...
% 鐢熸垚鍣０鍜岄殢鏈烘父璧?
r_walk = random_walk(D_T,D_N*D_T,D_N,0.00005);
rw = r_walk(1:D_N,1);
% rw = zeros(D_N,1);
...
roll2 = atan(D_GYR(1,3)/D_GYR(1,1))/pi*180;
Euler = Euler/pi*180;
Err = Euler(3,2:end)' - Y(1:D_N-1,8);


Z_Err3(:,1) = GPS_Y(1:10:end,3)'-XYZ(1,1:10:end);   %x
Z_Err3(:,2) = GPS_Y(1:10:end,1)'-XYZ(2,1:10:end);   %y
Z_Err3(:,3) = GPS_Y(1:10:end,2)'-XYZ(3,1:10:end);   %z
